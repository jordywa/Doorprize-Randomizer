 ...
 */var mp;function y1(){if(mp)return wr;mp=1;var n=Symbol.for("react.transitional.element"),i=Symbol.for("react.fragment");function o(l,u,f){var d=null;if(f!==void 0&&(d=""+f),u.key!==void 0&&(d=""+u.key),"key"in u){f={};for(var p in u)p!=="key"&&(f[p]=u[p])}else f=u;return u=f.ref,{$$typeof:n,type:l,key:d,ref:u!==void 0?u:null,props:f}}return wr.Fragment=i,wr.jsx=o,wr.jsxs=o,wr}var pp;function b1(){return pp||(pp=1,ac.exports=y1()),ac.exports}var zt=b1(),ic={exports:{}},lt={};/**
 * @license React
 * react.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
...
}`);Array.from({length:4},function(n,i){return"--life-line-phase".concat(i+1,"-color")});const FA=()=>{const[n,i]=_.useState(0),[o,l]=_.useState(!1),u=async()=>{l(!0);try{const p=await fetch("https://vdjt.jordy-server.my.id:5000/last-row");if(!p.ok)throw new Error("Response error");const h=await p.json();i(h[0])}catch(p){console.error("Error fetching data:",p)}l(!1)};_.useEffect(()=>{u()},[]);const f=async()=>{try{l(!0);const p=await fetch("https://vdjt.jordy-server.my.id:5000/increase",{method:"POST",headers:{"Content-Type":"application/json"}}),h=await p.json();if(!p.ok)throw new Error(h.error||"Something went wrong");i(Number(n)+1),l(!1)}catch(p){console.error(p)}},d=async()=>{try{l(!0);const p=await fetch("https://vdjt.jordy-server.my.id:5000/decrease",{method:"POST",headers:{"Content-Type":"application/json"}}),h=await p.json();if(!p.ok)throw new Error(h.error||"Something went wrong");i(Number(n)+1),l(!1)}catch(p){console.error(p)}};return zt.jsxs("div",{className:"flex flex-col items-center justify-items-center p-10 h-full",children:[o&&zt.jsx(QA,{variant:"bounce",color:"#31b7cc",size:"medium",text:"Sabar Ya",textColor:""}),!o&&zt.jsxs("div",{children:[zt.jsx("button",{onClick:f,className:"my-4 bg-sky-500 px-4 py-2 rounded-lg hover:bg-sky-600 text-white w-full",children:"+"}),zt.jsx("div",{className:"flex flex-row space-x-4 p-4 bg-gray-800 rounded-lg w-fit",children:zt.jsx("div",{className:"text-6xl font-bold text-white w-fit text-center",children:String(n).padStart(3,"0")})}),zt.jsx("button",{onClick:d,disabled:n<=0,className:"my-4 bg-red-400 px-4 py-2 rounded-lg hover:bg-red-500 text-white w-full",children:"-"})]})]})},JA=()=>zt.jsx(Hx,{basename:"/Doorprize-Randomizer/",children:zt.jsxs(yx,{children:[zt.jsx(Oc,{path:"/",element:zt.jsx(RA,{})}),zt.jsx(Oc,{path:"/manage",element:zt.jsx(FA,{})})]})});M1.createRoot(document.getElementById("root")).render(zt.jsx(_.StrictMode,{children:zt.jsx(JA,{})}));
